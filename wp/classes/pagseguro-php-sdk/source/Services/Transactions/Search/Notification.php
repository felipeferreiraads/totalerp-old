...
class Notification
{
    ...
    public static function search(Credentials $credentials, $code)
    {
        ...
        Logger::info("Begin", ['service' => 'Transactions.Notification.Code']);
        ...
        try {
            ...
            Logger::info(
                ...
                ['service' => 'Transactions.Notification.Code']
            );
            $http->get(
                self::request($connection, $code),
                20,
            ...
            Logger::error($exception->getMessage(), ['service' => 'Transactions.Notification.Code']);
            throw $exception;
        }
    }
    ...
    private static function request(Connection\Data $connection, $code)
    {
        return sprintf(
            "%s/%s/?%s",
        		$connection->buildNotificationTransactionRequestUrl(),
            $code,
